...
const ApprovalModal: React.FC<ModalProps> = ({ show, setShow, offerModal }) => {
  const handleClose = () => setShow(false);
  return (
    <StyledModal show={show} setShow={handleClose}>
      <Box className="content">
        <p className="modal-title border-bottom">
          {offerModal ? 'Confirm Offer' : 'Approve WETH'} <Close onClick={handleClose} />
        </p>
        <p className="description">We’ll ask your approval for the marketplace to access your WETH token. This is a one-time only operation.</p>
        <Button variant="filled" className="loader-btn">
